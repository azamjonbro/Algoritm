	...
	<div class="Certificate">
		<div class="certificate-topbar">
			...
			<h2 v-else class="not-found-title">
				Afsuski bizda bunday sertifikat topilmadi :(
			</h2>
			<p v-if="!certificateChecked || certificateFound">
				Sertifikat raqamingizni kiriting va natijani bir necha soniya ichida oling.
			</p>
			<p v-else class="not-found-text">
				Siz kiritgan sertifikat raqami to'g'ri emas yoki sertifikat hali ma'lumotlar bazasiga
				kiritilmagan bo'lishi mumkin. Iltimos, raqamni yana bir bor tekshirib ko'ring yoki
...
export default {
	...
	methods: {
		checkCertificate() {
			const validCertificates = ["ID-AT021", "ID-AT022"];
			this.certificateFound = validCertificates.includes(this.certificateId.trim());
			this.certificateChecked = true;
		}
	},
};
</script>
...
<style>
	...
	.certificate-search-wrapper .seach-field{
		...
		width: 242px;
		border-radius: 10px;
		border:  1px solid rgba(248, 218, 10, 1);
		height: 62px;
		color: white;
		font-family: Manrope;
		font-weight: 400;
